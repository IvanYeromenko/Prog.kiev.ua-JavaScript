...
const processCountries = res => { //здесь хранится вся наша информация с таблицы
    ...
    if(sortName) {
        sortCountries(sortName);
    }
    let countriesForAutocomplete = countries.map(el => el.name);
    $( "#tags" ).autocomplete({
...
$('thead td[data-sort]').on('click', e => {
    let sortName = $(e.currentTarget).attr('data-sort')
...
$('#tags').on("change ", function(e){
    ...
    var tagsCountry = $(e.currentTarget).val();
